...
(function ($) {
    ...
    function generateZip(){
        $.ajax({
            ...
            success: function () {
                window.location.href=document.getElementsByClassName('migration-button')[0].getAttribute('data-module-path')+"assets/migrationproserver.zip";
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                if (XMLHttpRequest.responseText.length > 0)
    ...
    function downloadLogFile(){
        window.open(document.getElementsByClassName('migration-warning-log')[0].getAttribute('data-warning-log'));
    }
    ...
    function displayWarning(warnings, step_number) {
        ...
        str_warning += '<p><a class="migration-warning-log" style="cursor:pointer" target="_blank" data-warning-log=' + warningLogPath + '>' + downloadLogText + '</a></p>';
        ...
        $('.migration-warning-log').on('click', function () {
            downloadLogFile();
        });
    }
    ...
    function initClickBothCartRuleCostumer() {
            $("label[for='entities_cart_rules_on']").click(function () {
                $("label[for='entities_customers_on']").click()
            }
          )
            $("label[for='entities_customers_off']").click(function () {
                $("label[for='entities_cart_rules_off']").click()
            }
          )
    }
    ...
    function bind_inputs() {
        ...
        $('.buttonTry').on('click', function () {
            if ($('.error.wizard_error').css('display') == 'block') {
                $('.error.wizard_error').hide();
            }
            $('.warning.wizard_error').fadeOut('fast');
            importData();
        });
    }
    ...
    function changePercentByEntityType(currentProcess) {
        var type = currentProcess.type;
        console.log("#process-" + type);
        var processParentElement = $("#process-" + type);
