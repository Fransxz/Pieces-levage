...
{block name="override_tpl"}
    <div class="row">
        <div id="step-3-clear" class="alert-success wizard_error" style="display:none">
            <ul>
                <li>{l s='Clear cache and re-build search index done.'  mod='migrationpro'}</li>
            </ul>
        </div>

        <div id="step-3-done" class="alert-success wizard_error" style="display:none">
            <ul>
                <li>{l s='Migration successfully done.'  mod='migrationpro'}</li>
            </ul>
        </div>
    </div>
    <div class="row">
        <div class="col-xs-12 col-sm-12 progress-container">
            <div class="panel kpi-container">
                <div class="panel-heading">
                    <i class="icon-AdminTools"></i> {l s='Migration Status'  mod='migrationpro'}
                </div>
                <div class="row">
                    {assign var="processCount" value={$processes|@count|escape:'htmlall':'UTF-8'}}

                    {if !$processes}
                    <h2>{l s='There is no new data on your source. =('  mod='migrationpro'}</h2>
                    {/if}
    ...
    <div class="row">
        <a href="#" class="buttonTry btn btn-success" style="display: none; float: right">{l s='Resume'  mod='migrationpro'}</a>
        <a href="#" class="buttonClear btn btn-success" style="display: none; float: right">{l s='Clear cache and Re-Buil Index'  mod='migrationpro'}</a>
    </div>
{/block}
